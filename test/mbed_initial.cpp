...
#define DYSKINESIA_COOLDOWN_CYCLES 2 // Suppress Tremor LED output for 2 cycles after Dyskinesia stops
...
bool was_dyskinesia_potentially_active = false; // Track previous potential state for cool-down trigger

DigitalOut led_tremor(LED_TREMOR_PIN, 0);
DigitalOut led_dyskinesia(LED_DYSKINESIA_PIN, 0);
...
// Changed signature back, logic moved inside
static void IndicateResults(bool tremor_currently_detected, bool dyskinesia_currently_detected);
...
int main()
{
    printf("--- Mbed Tremor/Dyskinesia Detection Start (Refactored Logic) ---\n");
...
static void InitializePeripherals(void)
{
    ...
    long long interval_us = (long long)((1.0f / ACTUAL_SAMPLING_FREQUENCY_HZ) * 1e6f);

    printf("Using Sensor ODR: %.2f Hz, Interval: %lld us\n", ACTUAL_SAMPLING_FREQUENCY_HZ, interval_us);
    printf("Freq Res: %.3f Hz\n", FREQUENCY_RESOLUTION);

    sampler_ticker.attach_us(callback(&SamplerISRHandler), interval_us);
    printf("Sampler ticker attached with %lld us interval\n", interval_us);
...
static void ProcessSensorData(void)
{
    ...
    for (uint16_t k = 1; k < FFT_SIZE / 2; k++)
    {
        ...
        {
            if (current_mag > max_tremor_mag) max_tremor_mag = current_mag;
        }
        else if (current_freq >= DYSKINESIA_LOW_HZ && current_freq <= DYSKINESIA_HIGH_HZ)
        {
            if (current_mag > max_dyskinesia_mag) max_dyskinesia_mag = current_mag;
        }
    }
    ...
    IndicateResults(tremor_currently_detected, dyskinesia_currently_detected);
...
// Refactored indication logic with corrected cool-down
static void IndicateResults(bool tremor_currently_detected, bool dyskinesia_currently_detected)
{
    // 1. Update Raw Consecutive Counts
    // Update tremor count based on current detection
    ...
    // Update dyskinesia count based on current detection
    ...
    // 2. Determine Potential Active States (based purely on counts)
    ...
    // Start cooldown if dyskinesia *was* potentially active but *is now* potentially inactive
    if (was_dyskinesia_potentially_active && !dyskinesia_potentially_active) {
        dyskinesia_cooldown_counter = DYSKINESIA_COOLDOWN_CYCLES;
    }

    // Update tracker for next cycle *before* applying cooldown suppression
    was_dyskinesia_potentially_active = dyskinesia_potentially_active;

    // Check if cooldown is active and decrement if it is
    bool tremor_output_suppressed = false;
    if (dyskinesia_cooldown_counter > 0) {
        tremor_output_suppressed = true; // Flag that tremor LED should be suppressed
        dyskinesia_cooldown_counter--;   // Decrement cooldown timer
    }

    // 4. Determine Final LED States
    bool final_dyskinesia_led_state = dyskinesia_potentially_active;
    bool final_tremor_led_state = tremor_potentially_active    // Tremor count must be high enough
                                  && !final_dyskinesia_led_state // Dyskinesia must NOT be active (priority)
                                  && !tremor_output_suppressed; // Cooldown must NOT be active

    // 5. Set Physical LEDs
    led_dyskinesia = final_dyskinesia_led_state ? 1 : 0;
    led_tremor = final_tremor_led_state ? 1 : 0;
}